...
namespace iCos5BACnetScheduleRT.View
{
  public partial class ViewWeeklySchedule : UserControl
  {
    ...
    public static readonly DependencyProperty SelectedListIndexProperty =
      DependencyProperty.Register("SelectedListIndex", typeof(int), typeof(ViewWeeklySchedule), new FrameworkPropertyMetadata(-1));
    ...
    private void DailyScheudleApply_Click(object sender, RoutedEventArgs e)
    {
      ...
      try
      {
        ...
        if (MessageBox.Show($"스케줄을 저장하시겠습니까?", "주간스케줄 저장", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
        {
          writeValue(((WeeklySchedule)DataContext).GetCode(), currentIndex);
        }
      }
      catch (Exception ex)
      {
