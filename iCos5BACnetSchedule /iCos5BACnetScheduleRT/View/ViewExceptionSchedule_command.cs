  ...
  public partial class ViewExceptionSchedule : UserControl
  {
    private void CommandBinding_Copy(object sender, ExecutedRoutedEventArgs e)
    {
      try
      {
        ...
        ObservableCollection<BooleanScheduleItemPair>[] itemPairs = ((ExceptionScheduleSet)DailySchedulesGrid.Items[0]).ScheduleItemPairs;
    ...
    private void CommandBinding_Paste(object sender, ExecutedRoutedEventArgs e)
    {
      try
      {
        ...
        if (retrievedData.GetDataPresent(DataFormats.Text))
        {
          ...
          ObservableCollection<BooleanScheduleItemPair>[] targetItems = ((ExceptionScheduleSet)selectedCells[0].Item).ScheduleItemPairs;
    ...
    private void CommandBinding_Delete(object sender, ExecutedRoutedEventArgs e)
    {
      try
      {
        ...
        {
          exceptionSchedule.ScheduleSet.ScheduleItemPairs[cell.Column.DisplayIndex].Clear();
          exceptionSchedule.ExceptionScheduleItems[cell.Column.DisplayIndex].Calendar = new CalendarEntry();
        }
      }
      catch (Exception ex)
      {
