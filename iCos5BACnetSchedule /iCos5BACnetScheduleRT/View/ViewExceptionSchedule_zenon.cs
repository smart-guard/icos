...
namespace iCos5BACnetScheduleRT.View
{
  public partial class ViewExceptionSchedule : UserControl
  {
    ...
    public static readonly DependencyProperty IsProgressProperty =
      DependencyProperty.Register("IsProgress", typeof(bool), typeof(ViewExceptionSchedule),
    ...
    {
      ((ExceptionSchedule)DataContext)?.ClearScheduleSet();
    }
    ...
    private void writeValue(string code, int prevIndex)
    {
      ...
      _celLogging.Info($"[{_variable.Name}]Update Exception Schedule");
      updateValue();
    }
    ...
    private void updateInvoke(string code, bool isInvalid)
    {
      ...
      {
        DataContext = new ExceptionSchedule(code, _config.DefaultExceptionPriority);
